...
impl Parser {
    ...
    fn parse_or(&mut self) -> Result<Expr, String> {
        ...
        while matches!(
            self.current().kind,
            TokenKind::Or | TokenKind::NullishCoalesce
        ) {
            let op = if matches!(self.current().kind, TokenKind::NullishCoalesce) {
                "??"
            } else {
                "||"
            };
    ...
    fn parse_arrow_body(&mut self, params: Vec<String>) -> Result<Expr, String> {
        if matches!(self.current().kind, TokenKind::LBrace) {
            ...
            Ok(Expr::Arrow {
                params,
                body: Box::new(ArrowBody::Block(body)),
            })
        } else {
            let expr = self.parse_assignment()?;
            Ok(Expr::Arrow {
                params,
                body: Box::new(ArrowBody::Expr(expr)),
            })
        }
    }
