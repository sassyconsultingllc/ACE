...
use super::parser::{ArrowBody, Expr, Parser, Stmt};
use super::value::{PromiseHandle, PromiseState, Value};
...
impl JsInterpreter {
    ...
    pub fn with_dom(mut self, dom: DomBridge) -> Self {
        ...
        self.globals
            .borrow_mut()
            .insert("document".to_string(), dom.create_document_object());
        self.globals
            .borrow_mut()
            .insert("window".to_string(), dom.create_window_object());
        self.dom = Some(dom);
        self
    }
    ...
    fn init_globals(&mut self) {
        ...
        math.borrow_mut()
            .insert("E".to_string(), Value::Number(std::f64::consts::E));
        self.globals
            .borrow_mut()
            .insert("Math".to_string(), Value::Object(math));
        ...
        self.globals.borrow_mut().insert(
            "parseInt".to_string(),
            Value::NativeFunction(native_parse_int),
        );
        self.globals.borrow_mut().insert(
            "parseFloat".to_string(),
            Value::NativeFunction(native_parse_float),
        );
        self.globals
            .borrow_mut()
            .insert("isNaN".to_string(), Value::NativeFunction(native_is_nan));
        self.globals.borrow_mut().insert(
            "isFinite".to_string(),
            Value::NativeFunction(native_is_finite),
        );
        self.globals
            .borrow_mut()
            .insert("Boolean".to_string(), Value::NativeFunction(native_boolean));
        self.globals
            .borrow_mut()
            .insert("Number".to_string(), Value::NativeFunction(native_number));
        self.globals.borrow_mut().insert(
            "String".to_string(),
            Value::NativeFunction(native_string_fn),
        );
        self.globals.borrow_mut().insert(
        ...
        self.globals.borrow_mut().insert(
            "Promise".to_string(),
            Value::NativeFunction(native_promise_constructor),
        );
        self.globals
            .borrow_mut()
            .insert("fetch".to_string(), Value::NativeFunction(native_fetch));
        self.globals
            .borrow_mut()
            .insert("alert".to_string(), Value::NativeFunction(native_alert));
        self.globals
            .borrow_mut()
            .insert("confirm".to_string(), Value::NativeFunction(native_confirm));
        self.globals
            .borrow_mut()
            .insert("prompt".to_string(), Value::NativeFunction(native_prompt));
    }
        ...
        match expr {
            ...
            Expr::Object(props) => {
                ...
                for (key, val_expr) in props {
                    map.borrow_mut()
                        .insert(key.clone(), self.evaluate(val_expr)?);
                }
                Ok(Value::Object(map))
            }
            ...
            Expr::New { callee, args: _ } => match callee.as_ref() {
                ...
                match obj {
                    Value::Array(arr) => {
                        if let Value::Number(n) = idx {
                            ...
                            let i = n as usize;
                            while arr_ref.len() <= i {
                                arr_ref.push(Value::Undefined);
                            }
                            arr_ref[i] = value;
                        }
                    }
                    Value::Object(map) => {
                        map.borrow_mut().insert(idx.to_string_value(), value);
                    }
                    _ => {}
                }
                Ok(())
    ...
    fn get_property(&mut self, obj: &Value, prop: &str) -> Result<Value, String> {
        match obj {
            ...
            Value::Object(map) => {
                if let Some(val) = map.borrow().get(prop) {
                    return Ok(val.clone());
                }
                Ok(Value::Undefined)
            }
            Value::Promise(_) => Ok(Value::BoundMethod {
                this: Box::new(obj.clone()),
                method_name: prop.to_string(),
            }),
            _ => Ok(Value::Undefined),
        }
    }
    ...
    fn string_property(&self, s: &str, prop: &str) -> Result<Value, String> {
        match prop {
            ...
            _ => Ok(Value::BoundMethod {
                this: Box::new(Value::String(s.to_string())),
                method_name: prop.to_string(),
            }),
        }
    }
    ...
    fn array_property(&self, arr: &Rc<RefCell<Vec<Value>>>, prop: &str) -> Result<Value, String> {
        match prop {
            ...
            _ => Ok(Value::BoundMethod {
                this: Box::new(Value::Array(arr.clone())),
                method_name: prop.to_string(),
            }),
        }
    }
    ...
    fn call_function(&mut self, callee: &Expr, args: &[Expr]) -> Result<Value, String> {
        ...
        if let Expr::Member { object, property } = callee {
            if let Expr::Identifier(name) = object.as_ref() {
                ...
                if name == "Math" {
                    return self.call_math_method(property, args);
                }
                if name == "JSON" {
                    return self.call_json_method(property, args);
                }
                if name == "Object" {
                    return self.call_object_method(property, args);
                }
                if name == "Array" {
                    return self.call_array_static_method(property, args);
                }
            }
        }
        ...
        match func {
            ...
            Value::BoundMethod { this, method_name } => {
                self.call_bound_method(&this, &method_name, &arg_vals)
            }
    ...
    pub fn call_value(&mut self, func: &Value, args: &[Value]) -> Result<Value, String> {
        match func {
            Value::Function {
                ...
                for (i, param) in params.iter().enumerate() {
                    self.scope_stack.last().unwrap().borrow_mut().insert(
                        param.clone(),
    ...
    fn call_array_method(
        ...
        match method {
            ...
            "shift" => Ok(if arr.borrow().is_empty() {
                Value::Undefined
            } else {
            ...
            "filter" => {
                ...
                for (i, v) in arr.borrow().iter().enumerate() {
                    if self
                        .call_value(&cb, &[v.clone(), Value::Number(i as f64)])?
                        .is_truthy()
                    {
                        result.push(v.clone());
                    }
            ...
            "includes" => {
                ...
                Ok(Value::Boolean(
                    arr.borrow().iter().any(|v| v.strict_equals(&search)),
                ))
            }
            "indexOf" => {
                let search = args.first().cloned().unwrap_or(Value::Undefined);
    ...
    fn call_json_method(&mut self, method: &str, args: &[Expr]) -> Result<Value, String> {
        match method {
            "stringify" => Ok(Value::String(
                format!("{:?}", self.evaluate(args.first().ok_or("No arg")?)?)
                    .replace("\"", "\\\""),
            )),
            "parse" => {
                let s = self
                    .evaluate(args.first().ok_or("No arg")?)?
                    .to_string_value();
                // Basic JSON parse
    ...
    fn call_array_static_method(&mut self, method: &str, args: &[Expr]) -> Result<Value, String> {
        match method {
            "isArray" => Ok(Value::Boolean(matches!(
                self.evaluate(args.first().ok_or("No arg")?)?,
                Value::Array(_)
            ))),
            _ => Ok(Value::Undefined),
        }
...
fn native_is_finite(args: Vec<Value>) -> Value {
    Value::Boolean(
        args.first()
            .map(|v| v.to_number())
            .unwrap_or(f64::NAN)
...
fn native_string_fn(args: Vec<Value>) -> Value {
    Value::String(
        args.first()
            .cloned()
            .unwrap_or(Value::Undefined)
...
fn native_set_timeout(_args: Vec<Value>) -> Value {
    Value::Number(0.0)
}
fn native_clear_timeout(_args: Vec<Value>) -> Value {
    Value::Undefined
}
fn native_set_interval(_args: Vec<Value>) -> Value {
    Value::Number(0.0)
}
...
fn native_confirm(_args: Vec<Value>) -> Value {
    Value::Boolean(true)
}
fn native_prompt(_args: Vec<Value>) -> Value {
    Value::Null
}
    ...
    #[cfg(feature = "fetch")]
    {
        match ureq::get(&url).call() {
            Ok(resp) => {
                ...
                obj.borrow_mut()
                    .insert("ok".to_string(), Value::Boolean(true));
                obj.borrow_mut()
                    .insert("status".to_string(), Value::Number(200.0));
                obj.borrow_mut()
                    .insert("text".to_string(), Value::String(body));
