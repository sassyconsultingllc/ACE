        ...
        match change.change_type {
            ChangeType::CreateFile => {
                ...
                let content = change
                    .content
                    .as_ref()
                    .ok_or_else(|| FileError::InvalidPath("No content for create".to_string()))?;
            ...
            ChangeType::UpdateFile => {
                let content = change
                    .content
                    .as_ref()
                    .ok_or_else(|| FileError::InvalidPath("No content for update".to_string()))?;
            ...
            ChangeType::RenameFile => {
                let new_path = change
                    .new_path
                    .as_ref()
                    .ok_or_else(|| FileError::InvalidPath("No new path for rename".to_string()))?;
            ...
            if path.is_file() {
                ...
                if let Some(fp) = file_pattern {
                    if let Some(ext) = path.extension() {
                        if !ext
                            .to_string_lossy()
                            .eq_ignore_ascii_case(fp.trim_start_matches("*."))
                        {
                            continue;
                        }
                    } else {
                ...
                if let Ok(content) = self.read_file(&path.to_string_lossy()) {
                    for (line_num, line) in content.lines().enumerate() {
                        if crate::fontcase::ascii_lower(line)
                            .contains(&crate::fontcase::ascii_lower(pattern))
                        {
                            matches.push(GrepMatch {
                                file: path.to_string_lossy().to_string(),
                                line: line_num + 1,
                                content: line.to_string(),
                                column: crate::fontcase::ascii_lower(line)
                                    .find(&crate::fontcase::ascii_lower(pattern)),
                            });
                        }
                    }
                }
