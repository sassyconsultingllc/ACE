...
use crate::ai::{load_runtime, run_help_query, AiRuntime, HelpQuery};
use crate::dom::{Document, NodeRef, NodeType};
...
use crate::sandbox::{
    Interaction, PageTrust, PopupDecision, PopupHandler, PopupRequest, Quarantine, QuarantinedFile,
    ReleaseStatus, SandboxManager,
};
use crate::script_engine::ScriptEngine;
use crate::style::{ComputedStyle, StyleEngine};
use crate::sync::{FamilyConfig, SecureSyncServer, SyncConfig, UserManager};
use crate::sync::{SyncCommand, SyncEvent};
use crate::ui::input::Key;
use crate::ui::popup::PopupReason;
use crate::ui::render::UIRenderer;
use crate::ui::{
    Edge, Focus, InputAction, InputManager, InputRect, InteractionType, MeaningfulInteraction,
    Modifiers, MouseButton, NetworkBar, PopupManager, RequestState, SandboxAction, SidebarState,
    Theme, TrustLevel, UiBounds, UI,
};
...
pub struct BrowserState {
    ...
    computed_styles:
        std::collections::HashMap<*const std::cell::RefCell<crate::dom::Node>, ComputedStyle>,
...
impl BrowserState {
    ...
    pub fn navigate(&mut self, url: &str) {
        ...
        // Fetch and parse
        self.network_bar
            .update_request(req_id, RequestState::Waiting);

        match self.http_client.fetch(&url) {
            Ok(response) => {
                self.network_bar
                    .update_request(req_id, RequestState::Receiving);
                self.network_bar
                    .add_bytes(req_id, response.body.len() as u64);
    ...
    fn submit_form_post(&mut self, url: &str, body: &str, content_type: &str) {
        ...
        let options =
            crate::protocol::FetchOptions::post(body).with_header("Content-Type", content_type);
    ...
    fn extract_title(doc: &Document) -> Option<String> {
        fn find_title(node: &crate::dom::Node) -> Option<String> {
            if node.node_type == NodeType::Element && node.tag_name.as_deref() == Some("title") {
                // Get text content
    ...
    fn scroll(&mut self, delta: i32) {
        ...
        if (self.scroll_y - old_scroll).abs() > 0 {
            if let Some(tab) = self.ui.tab_manager.active_tab_mut() {
                tab.sandbox
                    .record_interaction(MeaningfulInteraction::Scroll {
                        distance: delta.abs(),
                    });
            }
        }
    ...
    fn update_ui_bounds(&mut self) {
        ...
        self.ui_bounds.network_bar = InputRect::new(network_x, btn_y, network_width, btn_size);
    ...
    fn render(&mut self, buffer: &mut [u32]) {
        ...
        if let Some(ref layout) = self.layout {
            ...
            self.painter
                .paint_into(layout, buffer, self.ui.width, offset_x, offset_y);
        }

        // Draw sidebars (backgrounds)
        for edge in Edge::all() {
            self.ui_renderer
                .draw_sidebar(buffer, edge, &self.ui.sidebar_layout, theme);
        }
        ...
        if let Some(top) = self.ui.sidebar_layout.get(Edge::Top) {
            if top.is_visible() {
                ...
                self.ui_renderer.draw_network_bar_detailed(
                    buffer,
                    net_bounds,
                    &self.network_bar,
                    is_dark,
        ...
        if let Some(left) = self.ui.sidebar_layout.get(Edge::Left) {
            if left.is_expanded() {
                ...
                self.ui_renderer
                    .draw_tab_list(buffer, bounds, &self.ui.tab_manager, theme);
            }
        }
        ...
        if self.ui.help_pane_open && self.ui.ai.enabled {
            if let Some(right) = self.ui.sidebar_layout.get(Edge::Right) {
                if right.is_visible() {
                    let bounds =
                        right.bounds(self.ui.width, self.ui.height, &self.ui.sidebar_layout);
        ...
        self.ui_renderer
            .draw_tab_tiles(buffer, &self.ui.tab_manager, theme, content_rect);
        ...
        if let Some(ref server) = self.ui.sync_server {
            if server.is_running() {
                ...
                self.ui_renderer
                    .draw_sync_status(buffer, x, 10, server.client_count(), theme);
            }
        }
        ...
        if self.last_frame.elapsed() > Duration::from_secs(5) {
            if let Some(tab) = self.ui.tab_manager.active_tab() {
                ...
                self.ui
                    .tab_manager
                    .capture_preview(tab_id, buffer, self.ui.width, self.ui.height);
            }
    ...
    fn draw_network_bar(&self, buffer: &mut [u32], theme: &Theme) {
        ...
        // Background
        let bg_color = if theme.meta.name.contains("Dark") {
            0xff2a2a2a
        } else {
            0xffe0e0e0
        };
        ...
        if activity > 0.0 {
            ...
            let bar_color = if self.network_bar.is_active {
                0xff4a9eff
            } else {
                0xff44ff44
            };
            ...
            for dy in -radius..=radius {
                for dx in -radius..=radius {
                    if dx * dx + dy * dy <= radius * radius {
                        ...
                        if x >= 0
                            && y >= 0
                            && (x as u32) < self.ui.width
                            && (y as u32) < self.ui.height
                        {
                            let idx = (y as u32 * self.ui.width + x as u32) as usize;
                            if idx < buffer.len() {
    ...
    fn handle_sync_command(&mut self, client_id: u64, cmd: SyncCommand) {
        use crate::sync::protocol::{BrowserState as SyncBrowserState, SyncMessage, TabInfo};
    ...
    fn handle_page_click(&mut self, x: i32, y: i32) {
        ...
        match hit {
            HitTestResult::Link { url, .. } => {
                ...
                if let Some(tab) = self.ui.tab_manager.active_tab_mut() {
                    tab.sandbox
                        .record_interaction(MeaningfulInteraction::LinkClick);
            ...
            HitTestResult::FormInput { .. } => {
                ...
                if let Some(tab) = self.ui.tab_manager.active_tab_mut() {
                    tab.sandbox
                        .record_interaction(MeaningfulInteraction::FormInput);
                }
            }
            HitTestResult::FormButton { element_id } => {
                ...
                if let Some(tab) = self.ui.tab_manager.active_tab_mut() {
                    tab.sandbox
                        .record_interaction(MeaningfulInteraction::FormSubmit);
                }
        ...
        self.popup_manager
            .record_interaction(InteractionType::Click);
    }
    ...
    fn update_cursor(&mut self, x: i32, y: i32) -> CursorIcon {
        ...
        if self.ui.ai.show_help_button
            && self.ui.ai.enabled
            && self.ui_bounds.help_button.contains(x, y)
        {
            return CursorIcon::Pointer;
        }
    ...
    fn poll_timers(&mut self) {
        ...
        for (id, callback, repeat) in ready_timers {
            ...
            if let Err(e) = self
                .script_engine
                .execute_callback(callback.clone(), vec![])
            {
    ...
    fn save_session(&self) {
        ...
        let active_tab = self.ui.tab_manager.active_tab().and_then(|active| {
            self.ui
                .tab_manager
                .tabs()
                .iter()
        ...
        let session = SessionRestore {
            ...
            quarantine: self
                ...
                .map(|f| {
                    ...
                    let quarantined_at_ms = if now_ms >= age_ms {
                        now_ms - age_ms
                    } else {
                        now_ms
                    };
    ...
    pub fn register_family_device(&mut self, device_id: &str, name: &str, owner: &str) {
        ...
        println!(
            "Device registered: {} ({})",
            device.name,
            device.trust_level.description()
        );
    ...
    pub fn approve_device(&mut self, device_id: &str, approver_id: &str) {
        match self.family_config.approve_device(
            device_id,
            approver_id,
            crate::sync::TrustLevel::Trusted,
        ) {
    ...
    fn wire_api_surface(&self) {
        ...
        let _ =
            Self::quarantine_release as fn(&mut Self, &str) -> Result<std::path::PathBuf, String>;
        ...
        let _ = Self::handle_popup_request
            as fn(&mut Self, &str, Option<u32>, Option<u32>, bool) -> bool;
        ...
        let link = HitTestResult::Link {
            url: String::new(),
            text: String::new(),
        };
        let _ = link.describe();
        let _ = HitTestResult::FormInput { element_id: 0 };
        let _ = HitTestResult::Image {
            src: String::new(),
            alt: String::new(),
        };
        let _ = HitTestResult::Text {
            content: String::new(),
        };
    }
}
                    ...
                    WindowEvent::MouseInput {
                        ...
                        match btn_state {
                            ...
                            ElementState::Released => {
                                state
                                    .ui
                                    .mouse_up(state.input.mouse_x, state.input.mouse_y, btn);
                            }
                        }
                        window.request_redraw();
                    }
...
mod tests {
    ...
    fn test_engine_api_surface_linked() {
        ...
        let _ = BrowserState::download_update
            as fn(&BrowserState) -> Result<std::path::PathBuf, String>;
        ...
        let _ = BrowserState::quarantine_release
            as fn(&mut BrowserState, &str) -> Result<std::path::PathBuf, String>;
        ...
        let _ = BrowserState::handle_popup_request
            as fn(&mut BrowserState, &str, Option<u32>, Option<u32>, bool) -> bool;
        let _ =
            BrowserState::get_page_trust_status as fn(&BrowserState) -> Option<(PageTrust, String)>;
    }
}
