...
use crate::style::{
    AlignItems, ComputedStyle, Dimension, Display, FlexDirection, FlexWrap, JustifyContent,
};
...
impl LayoutEngine {
    ...
    pub fn layout(
        ...
        styles: &std::collections::HashMap<
            *const std::cell::RefCell<crate::dom::Node>,
            ComputedStyle,
        >,
    ) -> LayoutBox {
    ...
    fn layout_flex(&mut self, layout_box: &mut LayoutBox) {
        ...
        for child in &mut layout_box.children {
            ...
            child.bounds.width = if is_row {
                0.0
            } else {
                layout_box.content.width
            };
            child.bounds.height = if !is_row {
                0.0
            } else {
                layout_box.content.height
            };
    ...
    fn calculate_block_width(&mut self, layout_box: &mut LayoutBox) {
        ...
        layout_box.padding.width =
            layout_box.content.width + style.padding.left + style.padding.right;
    ...
    fn layout_block_children(&mut self, layout_box: &mut LayoutBox) {
        ...
        let has_inline = layout_box
            .children
            .iter()
            .any(|c| matches!(c.box_type, BoxType::Inline | BoxType::Text));
    ...
    fn calculate_block_height(&mut self, layout_box: &mut LayoutBox) {
        ...
        layout_box.padding.height =
            layout_box.content.height + style.padding.top + style.padding.bottom;
        layout_box.border.height =
            layout_box.padding.height + style.border.top + style.border.bottom;
        layout_box.margin.height =
            layout_box.border.height + style.margin.top + style.margin.bottom;
    ...
    fn layout_inline(&mut self, layout_box: &mut LayoutBox) {
        ...
        layout_box.content.x =
            layout_box.bounds.x + style.margin.left + style.border.left + style.padding.left;
        layout_box.content.y =
            layout_box.bounds.y + style.margin.top + style.border.top + style.padding.top;
    ...
    fn layout_text(&mut self, layout_box: &mut LayoutBox) {
        let font_size = if layout_box.style.font_size > 0.0 {
    ...
    pub fn hit_test(&self, layout: &LayoutBox, x: f32, y: f32) -> Option<NodeRef> {
        for child in layout.children.iter().rev() {
            if let Some(node) = self.hit_test(child, x, y) {
                return Some(node);
            }
        }
    ...
    pub fn find_layout_for_node<'a>(
        ...
        for child in &layout.children {
            if let Some(found) = self.find_layout_for_node(child, target) {
                return Some(found);
            }
        }
        None
