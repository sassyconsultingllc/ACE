...
use super::{ContentType, InteractionType, SecurityContext, ViolationSeverity};
...
impl QuarantinedFile {
    pub fn new(filename: String, source_url: String, content_type: String, data: Vec<u8>) -> Self {
    ...
    fn analyze(&mut self) {
        ...
        let dangerous_exts = [
            "exe", "msi", "bat", "cmd", "ps1", "vbs", "js", "jse", "wsf", "wsh", "scr", "pif",
            "com", "dll", "sys",
        ];
        ...
        for warning in &self.warnings {
            if warning.level == WarningLevel::Danger {
                self.security
                    .record_violation(&warning.message, ViolationSeverity::High);
            }
        }
    }
    ...
    pub fn release(
        ...
        match self.can_release() {
            ...
            ReleaseStatus::Waiting { seconds_remaining } => {
                return Err(format!("Wait {} more seconds", seconds_remaining));
            }
        ...
        std::fs::write(&path, &plaintext).map_err(|e| format!("Failed to write: {}", e))?;
...
fn mark_as_downloaded(path: &std::path::Path, source_url: &str) -> Result<(), String> {
    ...
    let zone = format!(
        "[ZoneTransfer]`r`nZoneId=3`r`nReferrerUrl={}`r`nHostUrl={}`r`n",
...
mod tests {
    ...
    fn test_dangerous_exe() {
        ...
        assert!(file
            .warnings
            .iter()
            .any(|w| w.level == WarningLevel::Danger));
    }
    ...
    fn test_double_extension_attack() {
        ...
        assert!(file
            .warnings
            .iter()
            .any(|w| w.message.contains("Hidden executable")));
    }
}
