...
impl Stylesheet {
            ...
            match c {
                ...
                ';' if brace_count == 0 => {
                    if let Some((name, value)) = current.split_once(':') {
                        props.insert(
                            crate::fontcase::ascii_lower(name.trim()),
        ...
        for part in selector.split(|c: char| c.is_whitespace() || c == '>' || c == '+' || c == '~')
        {
            ...
            for segment in part.split('.') {
                if segment.contains('#') {
                    ...
                    if part.starts_with('.') || segment != part.split('.').next().unwrap_or("") {
                        classes += 1;
                    } else {
                        elements += 1;
                    }
    ...
    pub fn matches_selector(node: &NodeRef, selector: &str) -> bool {
        ...
        if let Some(stripped) = selector.strip_prefix('#') {
            return n.get_id().as_deref() == Some(stripped);
        }
        if let Some(stripped) = selector.strip_prefix('.') {
            return n.has_class(stripped);
        }
        ...
        if !parts[0].is_empty() && parts[0] != tag {
            return false;
        }
        ...
        for (_i, part) in parts.iter().enumerate().skip(1) {
            ...
            if is_id {
                if n.get_id().as_deref() != Some(*part) {
                    return false;
                }
            } else if !n.has_class(part) {
                return false;
            }
        }
        true
...
impl StyleEngine {
    ...
    pub fn compute_style(&self, node: &NodeRef) -> ComputedStyle {
        ...
        matched.sort_by(|a, b| a.0.specificity.cmp(&b.0.specificity).then(a.1.cmp(&b.1)));
        ...
        for (name, value) in props {
            match name.as_str() {
                ...
                "background-color" | "background" => {
                    style.background_color = Self::parse_color(value)
                }
    ...
    fn parse_dimension(v: &str) -> Dimension {
        ...
        if v.ends_with('%') {
            return Dimension::Percent(v[..v.len() - 1].parse().unwrap_or(0.0));
        }
        if v.ends_with("px") {
            return Dimension::Px(v[..v.len() - 2].parse().unwrap_or(0.0));
        }
        if v.ends_with("em") {
            return Dimension::Em(v[..v.len() - 2].parse().unwrap_or(0.0));
        }
        if v.ends_with("rem") {
            return Dimension::Rem(v[..v.len() - 3].parse().unwrap_or(0.0));
        }
        if v.ends_with("vw") {
            return Dimension::Vw(v[..v.len() - 2].parse().unwrap_or(0.0));
        }
        if v.ends_with("vh") {
            return Dimension::Vh(v[..v.len() - 2].parse().unwrap_or(0.0));
        }
    ...
    fn parse_length(v: &str) -> f32 {
        let v = crate::fontcase::ascii_lower(v.trim());
        if v.ends_with("px") {
            v[..v.len() - 2].parse().unwrap_or(0.0)
        } else if v.ends_with("em") {
            v[..v.len() - 2].parse::<f32>().unwrap_or(0.0) * 16.0
        } else if v.ends_with("rem") {
            v[..v.len() - 3].parse::<f32>().unwrap_or(0.0) * 16.0
        } else if v.ends_with('%') {
            v[..v.len() - 1].parse::<f32>().unwrap_or(0.0)
        } else {
    ...
    fn parse_rgb_color(v: &str) -> Color {
        ...
        let parts: Vec<u8> = v[start..end]
            .split(',')
            .filter_map(|p| p.trim().parse().ok())
            .collect();
        if parts.len() >= 3 {
            Color::new(parts[0], parts[1], parts[2], 255)
        } else {
            Color::black()
        }
    }
