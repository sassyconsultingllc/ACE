...
use crate::ui::network_bar::{truncate_host, NetworkBar, NetworkBarColors, RequestState};
use crate::ui::tabs::TerminalColor;
use crate::ui::{Edge, Rect, SidebarLayout, TabManager, Theme, TileLayout};
...
impl UIRenderer {
    ...
    pub fn draw_nav_bar(
        ...
        self.fill_rect(
            buffer,
            bounds.x as i32,
            (bounds.y + bounds.height - 1) as i32,
            bounds.width,
            1,
            border,
        );
        ...
        // Forward button
        let fwd_color = if state.can_forward {
            text_color
        } else {
            text_dim
        };
    ...
    pub fn draw_tab_list(
        ...
        self.fill_rect(
            buffer,
            (bounds.x + bounds.width - 1) as i32,
            bounds.y as i32,
            1,
            bounds.height,
            border,
        ...
        for group in tab_manager.groups() {
            if y + 24 > bounds.y + bounds.height {
                break;
            }
            ...
            self.draw_text_truncated(
                ...
                &group_label,
                TextParams {
                    x: (bounds.x + 10) as i32,
                    y: (y + 16) as i32,
                    max_width: bounds.width - 20,
                    size: 11.0,
                    color: 0xffffffff,
                },
            );
        ...
        for tab in tab_manager.tabs() {
            if y + tab_height > bounds.y + bounds.height {
                break;
            }
    ...
    pub fn draw_tab_tiles(
        ...
        if !tab_manager.tile_view_active {
            return;
        }
        ...
        for (i, tab) in tabs.iter().enumerate() {
            ...
            self.draw_text(
                ...
                type_label,
                (x + 8) as i32,
                (y + preview_h + 8) as i32,
                10.0,
                hex_to_u32(&theme.colors.text_secondary),
            );
            ...
            let title_color = if is_selected {
                ...
                hex_to_u32(&theme.colors.text_primary)
            };
            self.draw_text_truncated(
                buffer,
                &tab.title,
        ...
        let _center_hit =
            tile_layout.hit_test(content_rect.width / 2, content_rect.height / 2, tabs.len());
        ...
        self.fill_rounded_rect(
            ...
            hex_to_u32(&theme.colors.surface),
            20,
        );
        self.draw_text(
            buffer,
    ...
    fn draw_terminal_output(
        &self,
        buffer: &mut [u32],
        terminal: &crate::ui::tabs::TerminalState,
        bounds: Rect,
    ) {
        ...
        for (i, line) in terminal.output.iter().skip(start).enumerate() {
            ...
            let color = if line.style.dim {
                blend_colors(0xff1a1a2e, fg, 0.5)
            } else {
                fg
            };
    ...
    pub fn draw_network_bar_detailed(
        &self,
        buffer: &mut [u32],
        bounds: Rect,
        ...
        let colors = if is_dark {
            NetworkBarColors::dark()
        } else {
            NetworkBarColors::light()
        };
        ...
        if activity > 0.0 {
            ...
            let bar_color = if network_bar.is_active {
                colors.active
            } else {
                colors.receiving
            };
        ...
        self.draw_text_truncated(
            ...
            TextParams {
                x: (bounds.x + 8) as i32,
                y: (bounds.y + 14) as i32,
                max_width: bounds.width - 16,
                size: 11.0,
                color: colors.text,
            },
        );
        ...
        if network_bar.expanded && bounds.height > 30 {
            ...
            for req in network_bar.visible_requests() {
                if y as u32 > bounds.y + bounds.height - 16 {
                    break;
                }
    ...
    pub fn draw_help_pane(
        ...
        if let Some(err) = error {
            ...
            for line in wrap_lines(err, 40) {
                ...
                y += 14;
                if y as u32 > bounds.y + bounds.height {
                    break;
                }
            }
...
mod tests {
    ...
    fn test_ui_renderer_full_draws() {
        ...
        renderer.draw_network_bar_detailed(
            &mut buffer,
            Rect {
                x: 0,
                y: 480,
