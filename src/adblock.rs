...
impl FilterList {
    ...
    pub fn describe(&self) -> String {
        format!(
            "FilterList[name={}, url={}, enabled={}, rules={}, updated={}]",
            self.name,
            self.url,
            self.enabled,
            self.rules.len(),
...
impl BlockStats {
    ...
    pub fn describe(&self) -> String {
        let top_domains: Vec<_> = self
            .blocked_by_domain
            .iter()
            .take(5)
            .map(|(d, c)| format!("{}:{}", d, c))
            .collect();
        let type_counts: Vec<_> = self
            .blocked_by_type
            .iter()
            .map(|(t, c)| format!("{:?}:{}", t, c))
            .collect();
        format!(
            "BlockStats[total={}, today={}, domains=[{}], types=[{}]]",
            self.total_blocked,
            self.blocked_today,
            top_domains.join(","),
            type_counts.join(",")
...
impl CompiledRule {
    ...
    pub fn describe(&self) -> String {
        format!(
            "CompiledRule[pattern={}, domains={}, except={}, types={}, 3p={:?}]",
            self.pattern,
...
impl CosmeticRule {
    ...
    pub fn describe(&self) -> String {
        format!(
            "CosmeticRule[sel={}, domains={}, except={}, style={}]",
            self.selector,
            self.domains.as_ref().map(|d| d.len()).unwrap_or(0),
            self.except_domains.as_ref().map(|d| d.len()).unwrap_or(0),
            self.style.is_some()
...
impl AdBlocker {
    ...
    pub fn describe(&self) -> String {
        ...
        let top_domains: Vec<_> = stats
            .blocked_by_domain
            .iter()
            .take(5)
            .map(|(d, c)| format!("{}={}", d, c))
            .collect();
        let top_types: Vec<_> = stats
            .blocked_by_type
            .iter()
            .take(5)
            .map(|(t, c)| format!("{:?}={}", t, c))
            .collect();
        let list_info: Vec<_> = self
            .filter_lists
            .iter()
            .map(|l| {
                format!(
            ...
            .collect();
        let compiled_info: Vec<_> = self
            .block_patterns
            .iter()
            .take(3)
            .map(|r| r.describe())
            .collect();
        let cosmetic_info: Vec<_> = self
            .cosmetic_rules
            .iter()
            .take(3)
            .map(|r| r.describe())
    ...
    pub fn should_block(&self, url: &str, page_domain: &str, resource_type: ResourceType) -> bool {
        if !self.enabled {
            return false;
        }
    ...
    fn rule_matches(
        ...
        if let Some(domains) = &rule.domains {
            let page_match =
                domains.contains(page_domain) || domain_matches_any(page_domain, domains);
            let req_match =
                domains.contains(request_domain) || domain_matches_any(request_domain, domains);
        ...
        if let Some(except) = &rule.except_domains {
            if except.contains(page_domain)
                || domain_matches_any(page_domain, except)
                || except.contains(request_domain)
                || domain_matches_any(request_domain, except)
            {
                return false;
            }
        }
...
impl AdBlockerUI {
    ...
    pub fn render(&mut self, ui: &mut eframe::egui::Ui) {
        ...
        if let Ok(mut blocker) = self.blocker.write() {
            ...
            let list_info: Vec<(String, usize, bool)> = blocker
                .filter_lists
                .iter()
...
mod tests {
    ...
    fn test_cosmetic_parsing() {
        ...
        if let FilterRule::CosmeticHide {
            selector, domains, ..
        } = rule
        {
