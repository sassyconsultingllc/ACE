...
use crate::file_handler::{FileContent, OpenFile, VideoContent};
...
impl VideoViewer {
    ...
    fn render_video_frame(&mut self, ui: &mut egui::Ui, video: &VideoContent, zoom: f32) {
        ...
        ui.painter()
            .rect_filled(rect, 0.0, Color32::from_rgb(15, 15, 20));

        // Film strip border effect
        let border_color = Color32::from_rgb(45, 50, 60);
        ui.painter()
            .rect_stroke(rect, 0.0, Stroke::new(2.0, border_color));
        ...
        if !self.is_playing {
            ...
            let points = vec![
                Pos2::new(
                    center.x - tri_size * 0.4 + offset,
                    center.y - tri_size * 0.6,
                ),
        ...
        if video.width > 0 && video.height > 0 {
            let res_label = if video.height >= 2160 {
                "4K"
        ...
        while gx < rect.right() {
            ...
            while gy < rect.bottom() {
                ...
                let hash = ((gx as u32).wrapping_mul(2654435761))
                    .wrapping_add((gy as u32).wrapping_mul(2246822519));
                if hash % 3 == 0 {
                    ui.painter()
                        .circle_filled(Pos2::new(gx, gy), 0.5, grain_color);
                }
                gy += grain_spacing;
            }
            gx += grain_spacing;
    ...
    fn render_controls_overlay(
        ...
        ui.painter()
            .rect_filled(progress_rect, 0.0, Color32::from_rgb(50, 55, 65));
        let filled_rect = Rect::from_min_size(
            progress_rect.min,
            Vec2::new(progress_rect.width() * progress, 3.0),
        );
        ui.painter()
            .rect_filled(filled_rect, 0.0, Color32::from_rgb(100, 180, 255));
        ...
        ui.allocate_ui_at_rect(controls_rect.shrink2(Vec2::new(10.0, 8.0)), |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    // Fullscreen toggle
                    let fs_icon_name = if self.is_fullscreen {
                        "exit-fullscreen"
                    } else {
                        "fullscreen"
                    };
                    ...
                    // Volume
                    let vol_icon_name = if self.is_muted {
                        "volume-mute"
                    } else {
                        "volume-high"
                    };
    ...
    fn render_info_overlay(&self, ui: &mut egui::Ui, video: &VideoContent) {
        ...
        ui.allocate_ui_at_rect(overlay_rect.shrink(12.0), |ui| {
            ui.label(
                RichText::new("Video Information")
                    .size(14.0)
                    .strong()
            ...
            egui::Grid::new("video_info_overlay_grid")
                ...
                .show(ui, |ui| {
                    ...
                    if video.width > 0 {
                        ...
                        ui.label(
                            RichText::new(format!("{}x{}", video.width, video.height))
                                .small()
                                .color(value_color),
                        );
                        ui.end_row();
                    ...
                    if video.frame_rate > 0.0 {
                        ...
                        ui.label(
                            RichText::new(format!("{:.2} fps", video.frame_rate))
                                .small()
                                .color(value_color),
                        );
                        ui.end_row();
                    ...
                    ui.label(
                        RichText::new(format_time(video.duration))
                            .small()
                            .color(value_color),
                    );
                    ui.end_row();
                    ...
                    if let Some(bitrate) = video.bitrate {
                        ...
                        if bitrate > 1_000_000 {
                            ui.label(
                                RichText::new(format!("{:.1} Mbps", bitrate as f64 / 1_000_000.0))
                                    .small()
                                    .color(value_color),
                            );
                        } else {
                    ...
                    ui.label(
                        RichText::new(format!("{}x", self.playback_rate))
                            .small()
                            .color(value_color),
                    );
                    ui.end_row();
