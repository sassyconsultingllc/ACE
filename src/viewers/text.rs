            ...
            egui::ComboBox::from_id_salt("theme_selector")
                ...
                .show_ui(ui, |ui| {
                    for theme_name in self.theme_set.themes.keys() {
                        if ui
                            .selectable_label(self.current_theme == *theme_name, theme_name)
                            .clicked()
                        {
        ...
        egui::ScrollArea::both()
            ...
            .show(ui, |ui| {
                ...
                if let Some(cursor) = response.cursor_range {
                    ...
                    self.cursor_column =
                        text_before.lines().last().map(|l| l.len() + 1).unwrap_or(1);
                }
            });
    }
                        ...
                        ui.vertical(|ui| {
                            ...
                            for line in &lines {
                                    ...
                                    highlighter.highlight_line(line, &self.syntax_set)
                                {
                                    ...
                                    ui.label(
                                        RichText::new(*line).font(FontId::monospace(font_size)),
                                    );
                                }
                            }
                        });
