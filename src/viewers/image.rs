...
impl ImageViewer {
    ...
    pub fn apply_resize(&mut self, width: u32, height: u32) {
        if let Some(img) = &self.working_image {
            if width > 0 && height > 0 {
                self.working_image = Some(img.resize_exact(
                    width, 
                    height, 
    ...
    pub fn export(&self, path: &PathBuf, format: ExportFormat, quality: u8) -> Result<(), String> {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            if self.show_adjustments {
                ui.collapsing("🎨 Adjustments", |ui| {
                    ...
                    ui.label("Brightness");
                    changed |= ui.add(egui::Slider::new(&mut self.adjustments.brightness, -100.0..=100.0)).changed();
                    
                    ui.label("Contrast");
                    changed |= ui.add(egui::Slider::new(&mut self.adjustments.contrast, -100.0..=100.0)).changed();
                    
                    ui.label("Saturation");
                    changed |= ui.add(egui::Slider::new(&mut self.adjustments.saturation, -100.0..=100.0)).changed();
                    
                    ui.label("Hue");
                    changed |= ui.add(egui::Slider::new(&mut self.adjustments.hue, -180.0..=180.0)).changed();
                    
                    ui.label("Gamma");
                    changed |= ui.add(egui::Slider::new(&mut self.adjustments.gamma, 0.1..=3.0)).changed();
            ...
            if self.show_filters {
                ui.collapsing("✨ Filters", |ui| {
                    ...
                    for (filter, name) in filters {
                        if ui.selectable_label(self.current_filter == filter, name).clicked() {
            ...
            egui::ScrollArea::both()
                ...
                .show(ui, |ui| {
                    ...
                    if self.current_tool == EditTool::Crop {
                        self.handle_crop_interaction(&response, &painter, rect, img_size, display_size);
                    }
                });
        } else {
    ...
    fn handle_crop_interaction(
        ...
        if response.dragged() {
            if let (Some(pos), Some(crop)) = (response.interact_pointer_pos(), &mut self.crop_selection) {
                let local = (pos - image_rect.min) * scale;
                crop.end = Pos2::new(
        ...
        if let Some(crop) = &self.crop_selection {
            ...
            painter.rect_filled(
                Rect::from_min_max(Pos2::new(image_rect.min.x, crop_rect.min.y), Pos2::new(crop_rect.min.x, crop_rect.max.y)),
                0.0, dark
            );
            painter.rect_filled(
                Rect::from_min_max(Pos2::new(crop_rect.max.x, crop_rect.min.y), Pos2::new(image_rect.max.x, crop_rect.max.y)),
                0.0, dark
            );
            ...
            .show(ui.ctx(), |ui| {
                ui.horizontal(|ui| {
                    ...
                    egui::ComboBox::from_id_salt("export_format")
                        ...
                        .show_ui(ui, |ui| {
                            ui.selectable_value(&mut self.export_format, ExportFormat::Png, "PNG (lossless)");
                            ...
                            ui.selectable_value(&mut self.export_format, ExportFormat::Tiff, "TIFF");
                            ui.selectable_value(&mut self.export_format, ExportFormat::Gif, "GIF");
                        });
                });
                
                if self.export_format == ExportFormat::Jpeg || self.export_format == ExportFormat::WebP {
                    ui.horizontal(|ui| {
                        ui.label("Quality:");
                ...
                if let Some((w, h)) = self.dimensions() {
                    ui.label(format!("Size: {}×{} pixels", w, h));
                }
                ...
                ui.horizontal(|ui| {
                    if ui.button("Save As...").clicked() {
                        if let Some(path) = native_dialog::FileDialog::new()
                            ...
                            .flatten()
                        {
                            if let Err(e) = self.export(&path, self.export_format, self.export_quality) {
