...
impl Model3DViewer {
    ...
    fn render_toolbar(
        ...
        ui.horizontal(|ui| {
            ...
            ui.label("Render:");
            if ui
                .selectable_label(self.render_mode == RenderMode::Wireframe, "Wire")
                .clicked()
            {
                self.render_mode = RenderMode::Wireframe;
            }
            if ui
                .selectable_label(self.render_mode == RenderMode::Solid, "Solid")
                .clicked()
            {
                self.render_mode = RenderMode::Solid;
            }
            if ui
                .selectable_label(self.render_mode == RenderMode::SolidWireframe, "Both")
                .clicked()
            {
                self.render_mode = RenderMode::SolidWireframe;
            }
            if ui
                .selectable_label(self.render_mode == RenderMode::Points, "* Points")
                .clicked()
            {
            ...
            // Reset view
            if icons
                .text_button(ui, "reload", "Reset View", "Reset camera")
                .clicked()
            {
    ...
    fn render_3d_viewport(&mut self, ui: &mut egui::Ui, model: &Model3DContent, global_zoom: f32) {
        ...
        let scale =
            (viewport_size.x.min(viewport_size.y) * 0.4) / max_size * self.zoom * global_zoom;
        ...
        let projected: Vec<(Pos2, f32)> = model
            .vertices
            .iter()
    ...
    fn draw_solid(
        ...
        let mut sorted_faces: Vec<(&Face3D, f32)> = model
            .faces
            .iter()
            .map(|face| {
                let avg_z: f32 = face
    ...
    fn draw_axes(&self, painter: &egui::Painter, center: Pos2, length: f32) {
        ...
        // Y axis (green)
        let y_end = Pos2::new(center.x, center.y - length * rot_x.cos());
