...
impl ChemicalViewer {
    ...
    fn render_toolbar(
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .selectable_label(self.render_mode == RenderMode::Wireframe, "Wireframe")
                .clicked()
            {
                self.render_mode = RenderMode::Wireframe;
            }
            if ui
                .selectable_label(self.render_mode == RenderMode::Spacefill, "Spacefill")
                .clicked()
            {
            ...
            // Reset view
            if icons
                .text_button(ui, "reload", "Reset View", "Reset view")
                .clicked()
            {
                self.rotation_x = 0.0;
                self.rotation_y = 0.0;
    ...
    fn render_3d_view(&mut self, ui: &mut egui::Ui, chem: &ChemicalContent, global_zoom: f32) {
        ...
        if response.clicked() {
            if let Some(click_pos) = response.interact_pointer_pos() {
                self.selected_atom = projected_atoms
                    .iter()
            ...
            .show_inside(ui, |ui| {
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    let chains: std::collections::HashSet<_> =
                        chem.atoms.iter().map(|a| a.chain).collect();
    ...
    fn draw_axis_indicator(&self, painter: &egui::Painter, rect: Rect) {
        ...
        // Y axis (green)
        let y_end = Pos2::new(origin.x, origin.y - length * rot_x_rad.cos());
