...
impl DownloadManager {
    ...
    pub fn start_download(&self, url: &str, suggested_filename: Option<&str>) -> Result<Uuid> {
        ...
        let mut download = Download::new(url.to_string(), filename, save_path.clone());
        ...
        let result = (|| -> Result<()> {
            ...
            // Update total bytes
            {
                ...
                if let Some(d) = downloads.iter_mut().find(|d| d.id == id) {
                    ...
                    d.mime_type = response.header("content-type").map(String::from);
                }
            }
    ...
    fn generate_unique_path(&self, filename: &str) -> PathBuf {
        ...
        // Fallback with UUID
        self.download_dir
            .join(format!("{}_{}{}", stem, Uuid::new_v4(), ext))
    }
    ...
    pub fn clear_finished(&self) {
        let mut downloads = self.downloads.lock().unwrap();
        downloads
            .retain(|d| d.state == DownloadState::Downloading || d.state == DownloadState::Pending);
    }
