...
use crate::browser::{BookmarkManager, DownloadManager, HistoryManager, Tab, TabContent, TabId};
...
impl BrowserEngine {
    ...
    pub fn new_tab_with_url(&mut self, url: &str) -> TabId {
        ...
        self.pending_navigations
            .insert(id, PendingNavigation { url });
        self.tabs.push(tab);
        self.active_tab_index = self.tabs.len() - 1;
        self.update_address_bar();
    ...
    pub fn navigate(&mut self, url: &str) {
        ...
        if let Some(id) = tab_id {
            self.pending_navigations
                .insert(id, PendingNavigation { url: url.clone() });
    ...
    fn handle_message(&mut self, msg: WebViewMessage) {
        match msg {
            ...
            WebViewMessage::NavigationStateChanged {
                tab_id,
                can_go_back,
                can_go_forward,
            } => {
            ...
            WebViewMessage::DownloadStarted {
                url,
                suggested_filename,
            } => {
